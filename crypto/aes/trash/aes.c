...
void aes_encrypt(aes_ctx_t *ctx, unsigned char input[16], unsigned char output[16])
{
	...
	print_block("before first step", (void*)ctx->state );
	
	
	aes_addroundkey(ctx, 0);
	//DEBUG
	print_block("first round key", (void*)ctx->keysched );
	print_block("first step", (void*)ctx->state );
	...
	for(i = 1; i < ctx->rounds; i++) {
		...
		aes_subbytes(ctx);
		//DEBUG
		print_block("after sub_bytes()", (void*)ctx->state );
		aes_shiftrows(ctx);
		//DEBUG
		print_block("after shift_rows()", (void*)ctx->state );
		aes_mixcolumns(ctx);
		//DEBUG
		print_block("after mix_columns()", (void*)ctx->state );
		print_block("round_key:", (void*)(ctx->keysched+i*4));
		aes_addroundkey(ctx, i);
		//DEBUG
		print_block("after add_round_key()", (void*)ctx->state );
	}
	...
	print_block("last round key", (void*)(ctx->keysched+ctx->rounds) );
	print_block("last step", (void*)(ctx->state) );
